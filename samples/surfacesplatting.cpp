...
int main(int argc, char* argv[]) {
    ...
    auto preRender = [&app](Engine*, View*, Scene*, Renderer* renderer) {
        ...
        if (once) {
            ...
            std::cout << "depthRT    ="
                      << app.depthRenderTarget->getTexture(RenderTarget::AttachmentPoint::DEPTH)
                      << std::endl;
        }
    };
...
static void initVbIb(Engine* vEngine, SApp& vApp) {
    ...
    std::cout << "isloadRsfSuccess : "
              << loadRsfFile(FilamentApp::getRootAssetsPath() + pRsfPath, vApp._Surfels)
              << std::endl;
    ...
    vApp._pIb->setBuffer(*vEngine,
            IndexBuffer::BufferDescriptor(vApp._KIndices.data(), byteCount, nullptr));
}
static void createSurfaceSplat(Engine* vEngine, Scene* vScene, SApp& vApp) {
    // 创建材质
