...
int main(int argc, char* argv[])
{
    ...
    auto setup = [&app](Engine* engine, View* view, Scene* scene) {
        ...
        // 加载rsf文件并初始化Vb,Ib
        InitVb(engine, app);
        createPointRender(engine, scene, app);
...
static void InitVb(Engine* engine, App& app) {
    ...
    for (size_t i = 0; i < pointNumber; i++) {
        minx = std::min(minx, app.mSurfels[i].position.x);
        miny = std::min(miny, app.mSurfels[i].position.y);
        minz = std::min(minz, app.mSurfels[i].position.z);
        maxx = std::max(maxx, app.mSurfels[i].position.x);
        maxy = std::max(maxy, app.mSurfels[i].position.y);
        maxz = std::max(maxz, app.mSurfels[i].position.z);
    }
    ...
    app.box.center = (mn + mx) * 0.5f;
    app.box.halfExtent = (mx - mn) * 0.5f;

    for (size_t i = 0; i < pointNumber; i++) {
        app.mSurfels[i].position.x = 2.0f * (app.mSurfels[i].position.x - minx) / (maxx-minx)-1.0f;
        app.mSurfels[i].position.y = 2.0f * (app.mSurfels[i].position.y - miny) / (maxy-miny)-1.0f;
        app.mSurfels[i].position.z = 2.0f * (app.mSurfels[i].position.z - minz) / (maxz-minz)-1.0f;
    }


    app.vb = VertexBuffer::Builder()
                     .vertexCount(static_cast<uint32_t>(app.mSurfels.size()))
                     .bufferCount(1)
                     .attribute(VertexAttribute::POSITION, 0, AttributeType::FLOAT3,
                             offsetof(SSurfel, position), sizeof(SSurfel))
                     .attribute(VertexAttribute::CUSTOM1, 0, AttributeType::FLOAT3,
                             offsetof(SSurfel, normal), sizeof(SSurfel))
                     .attribute(VertexAttribute::CUSTOM0, 0, AttributeType::FLOAT,
                             offsetof(SSurfel, radius), sizeof(SSurfel))
                     .attribute(VertexAttribute::COLOR, 0, AttributeType::FLOAT4,
                             offsetof(SSurfel, color), sizeof(SSurfel))
                     .build(*engine);
    app.vb->setBufferAt(*engine, 0,
            VertexBuffer::BufferDescriptor(app.mSurfels.data(),
                    app.mSurfels.size() * sizeof(SSurfel), nullptr));
...
static void createPointRender(Engine* engine, Scene* scene, App& app) {
    ...
    app.matInstance->setParameter("pointSizeScale", 10.0f);

    RenderableManager::Builder(1)
            .boundingBox(app.box)
            .material(0, app.matInstance)
            .geometry(0, RenderableManager::PrimitiveType::POINTS, app.vb, app.ib, 0, app.mSurfels.size())
            .culling(false)
...
// 实现
void createSurfaceSplat(Engine* engine, Scene* scene, App& app)
{
    // 创建材质
    app.mat = Material::Builder()
...
void createOffscreenSplat(Engine* engine, Scene* scene, View* mainView, App& app) {
    //// 1️ 取主窗口尺寸
...
static void setupCamera(View* view, App& app) {
    ...
    std::cout << "camerapos:" << app.camera->getPosition().x << " " << app.camera->getPosition().y << " "
